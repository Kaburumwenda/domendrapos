import { ref } from 'file://D:/Projects/MY-APPS-2/DomendraPOS/frontend/node_modules/vue/index.mjs';
...
const CMD = {
  ...
  ALIGN_RIGHT: new Uint8Array([ESC, 97, 2]),
  BOLD_ON: new Uint8Array([ESC, 69, 1]),
  BOLD_OFF: new Uint8Array([ESC, 69, 0]),
  DOUBLE_WIDTH_ON: new Uint8Array([GS, 33, 32]),
  DOUBLE_WIDTH_OFF: new Uint8Array([GS, 33, 0]),
  DOUBLE_HEIGHT_ON: new Uint8Array([GS, 33, 16]),
  DOUBLE_HEIGHT_OFF: new Uint8Array([GS, 33, 0]),
  SIZE_NORMAL: new Uint8Array([GS, 33, 0]),
  SIZE_DOUBLE: new Uint8Array([GS, 33, 17]),
  SIZE_DOUBLE_HEIGHT: new Uint8Array([GS, 33, 16]),
  CUT_PAPER: new Uint8Array([GS, 86, 0]),
  CUT_PARTIAL: new Uint8Array([GS, 86, 1]),
  FEED_LINES: (n) => new Uint8Array([ESC, 100, n]),
  FEED_PAPER: (n) => new Uint8Array([ESC, 74, n]),
  OPEN_DRAWER: new Uint8Array([ESC, 112, 0, 25, 250]),
  SET_CODEPAGE: (n) => new Uint8Array([ESC, 116, n])
};
...
function itemLine(name, qty, price, total, width, symbol) {
  name = String(name != null ? name : "");
  const qtyStr = String(qty != null ? qty : 0);
...
function buildEscPosReceipt(data, config = { paperWidth: 48, codepage: 0 }) {
...
function useEscPos() {
  ...
  async function connectUsb() {
    ...
    try {
      error.value = null;
      const device_ = await (void 0).usb.requestDevice({ filters: USB_FILTERS });
      await device_.open();
  ...
  async function connectBluetooth() {
    ...
    try {
      error.value = null;
      const device_ = await (void 0).bluetooth.requestDevice({
        filters: [
          { services: [6384] },
          { services: [4353] },
          { namePrefix: "Printer" },
        ...
        optionalServices: [6384, 4353]
      });
      const server = await device_.gatt.connect();
      const service = await server.getPrimaryService(6384);
      const characteristic = await service.getCharacteristic(10993);
      device.value = device_;
      device.value._characteristic = characteristic;
      connected.value = true;
      return true;
    } catch (e) {
      if (e.name === "NotFoundError") {
  ...
  async function printRaw(data) {
    ...
    try {
      if ("transferOut" in device.value) {
        const usbDevice = device.value;
        const iface = (_a = usbDevice.configuration) == null ? void 0 : _a.interfaces[0];
        const endpoint = (_b = iface == null ? void 0 : iface.alternates[0]) == null ? void 0 : _b.endpoints.find((e) => e.direction === "out");
        if (endpoint) {
      ...
      const char = device.value._characteristic;
      if (char) {
        const chunkSize = 180;
  ...
  function printHtmlFallback(data) {
    const w = 320;
    const html = buildHtmlReceipt(data, w);
    const win = (void 0).open("", "_blank", `width=${w + 40},height=600`);
    if (!win) {
  ...
  async function smartPrint(data, config) {
    if (connected.value && device.value) {
  ...
  async function disconnect() {
    if (device.value) {
      try {
        if ("usb" in void 0 && device.value instanceof USBDevice) {
...
function buildHtmlReceipt(data, width) {
  const sym = data.currencySymbol || "$";
  const fmt = (v) => formatMoney(v, sym);
...
export { useEscPos as u };
//# sourceMappingURL=useEscPos-CPWtz9qm.mjs.map
