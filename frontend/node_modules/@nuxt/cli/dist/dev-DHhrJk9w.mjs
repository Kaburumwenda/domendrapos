...
/* build-hook-start *//*00001*/try { require('/home/alpha/.vscode/extensions/wallabyjs.console-ninja-1.0.540/out/buildHook/index.js').default({tool: 'nuxt', checkSum: '2050765c6f7099e70674f0Hx4dGB9ZAwMHAwoPAQFVBwNSXwMC', mode: 'build', condition: true}); } catch(cjsError) { try { import('file:///home/alpha/.vscode/extensions/wallabyjs.console-ninja-1.0.540/out/buildHook/index.js').then(m => m.default.default({tool: 'nuxt', checkSum: '2050765c6f7099e70674f0Hx4dGB9ZAwMHAwoPAQFVBwNSXwMC', mode: 'build', condition: true})).catch(esmError => {}) } catch(esmError) {}}/* build-hook-end */
...
import { i as withNodePath, t as loadKit } from "./kit-gdbmFADU.mjs";
import { n as showBanner } from "./banner-DRnZZ6MZ.mjs";
...
*/
function attachViteHmrServer(server, hmrServer) {
	const target = server;
	target.ws = {
		protocol: void 0,
		...target.ws,
...
var NuxtDevServer = class extends EventEmitter {
	...
	async #initializeNuxt(reload) {
		...
		if (!process.env.NUXI_DISABLE_VITE_HMR) this.#currentNuxt.hooks.hook("vite:extend", ({ config }) => {
			if (config.server) attachViteHmrServer(config.server, this.listener.server);
		});
	...
	#watchConfig() {
		this.#configWatcher = createConfigWatcher(this.#cwd, this.options.dotenv.fileName, () => this.emit("restart"), (file) => this.loadDebounced(true, `${file} updated`), getLocalLayerDirs(this.#currentNuxt?.options._layers ?? [], this.#cwd));
	}
};
...
function getLocalLayerDirs(layers, cwd) {
	const root = resolve(cwd);
	const dirs = /* @__PURE__ */ new Set();
	for (const layer of layers) {
		const dir = layer.cwd || layer.config?.rootDir;
		const resolved = dir && resolve(dir);
		if (resolved && resolved !== root && resolved.startsWith(`${root}/`) && !resolved.includes("/node_modules/")) dirs.add(resolved);
	}
	return [...dirs];
}
function createConfigWatcher(cwd, dotenvFileName = ".env", onRestart, onReload, layerDirs = []) {
	const dotenvFileNames = new Set(Array.isArray(dotenvFileName) ? dotenvFileName : [dotenvFileName]);
	const closers = [watchConfigDir(cwd, onReload, (file) => dotenvFileNames.has(file) && onRestart()), ...layerDirs.map((dir) => watchConfigDir(dir, onReload))];
	return () => {
		for (const close of closers) close();
	};
}
function watchConfigDir(dir, onReload, onFile) {
	const fileWatcher = new FileChangeTracker();
	fileWatcher.prime(dir);
	const watcher = watch(dir);
	let configDirWatcher = existsSync(join(dir, ".config")) ? createConfigDirWatcher(dir, onReload) : void 0;
	watcher.on("change", (_event, file) => {
		if (!file || !fileWatcher.shouldEmitChange(resolve(dir, file))) return;
		onFile?.(file);
		if (RESTART_RE.test(file)) onReload(file);
		if (file === ".config") configDirWatcher ||= createConfigDirWatcher(dir, onReload);
	});
	return () => {
		watcher.close();
		configDirWatcher?.();
