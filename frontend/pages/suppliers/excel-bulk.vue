        ...
        <div class="d-flex align-center ga-3">
          ...
          <div>
            ...
            <div class="text-body-2 text-medium-emphasis">
              Bulk upload &amp; export suppliers via .xlsx workbook
            </div>
          </div>
        </div>
      </v-col>
      <v-col cols="12" sm="5" class="d-flex justify-end ga-2 flex-wrap">
        <v-btn variant="outlined" prepend-icon="mdi-truck-outline" @click="navigateTo('/suppliers')">
          View Suppliers
        </v-btn>
      </v-col>
    </v-row>
      ...
      <v-window-item value="import">
        ...
        <v-card rounded="xl" flat border class="pa-6 mb-6">
          ...
          <v-alert type="info" variant="tonal" density="compact" rounded="lg">
            Each row maps to one supplier. <strong>Supplier Code is optional</strong> — blank codes are auto-generated. Existing codes are updated.
          </v-alert>
        </v-card>
        ...
        <v-card rounded="xl" flat border class="pa-6 mb-6">
          ...
          >
            ...
            <template v-if="!selectedFile">
              ...
              <div class="text-body-2 text-medium-emphasis">Only .xlsx files · max 20MB · Supplier Code optional</div>
            </template>
            <template v-else>
          ...
          <v-card v-if="preview" rounded="xl" flat border class="pa-6 mb-6">
              ...
              <v-table density="compact" class="preview-table">
                ...
                <tbody>
                  <tr v-for="(row, i) in previewRows" :key="i"
                    ...
                    <td><v-text-field v-model="row.supplier_code" variant="outlined" density="compact" hide-details placeholder="auto" class="preview-input" /></td>
                    <td><v-text-field v-model="row.name" variant="outlined" density="compact" hide-details class="preview-input" /></td>
                    <td><v-text-field v-model="row.contact_person" variant="outlined" density="compact" hide-details class="preview-input" /></td>
                    ...
                    <td><v-text-field v-model="row.payment_terms" variant="outlined" density="compact" hide-details class="preview-input" /></td>
                    <td><v-text-field v-model="row.lead_time_days" type="number" step="1" variant="outlined" density="compact" hide-details class="preview-input" /></td>
            ...
            <div class="d-flex align-center ga-3 mt-5 flex-wrap">
              <v-btn color="success" prepend-icon="mdi-content-save" :loading="saving"
                ...
                Save {{ previewRows.length }} Supplier(s)
              </v-btn>
        ...
        <v-slide-y-transition>
          <v-card v-if="importResult" rounded="xl" flat border class="pa-6" :style="resultBorderStyle">
            <div class="d-flex align-center ga-3 mb-5 flex-wrap">
              ...
              <v-spacer />
              <v-btn v-if="importResult.failed === 0" color="success" prepend-icon="mdi-truck-outline" @click="navigateTo('/suppliers?imported=1')">
                View Suppliers
              </v-btn>
        ...
        <v-row>
          <v-col cols="12" lg="7">
            <v-card rounded="xl" flat border class="pa-6 h-100">
              <div class="d-flex align-center ga-3 mb-5">
                ...
                <div>
                  <div class="text-h6 font-weight-bold">Export Suppliers to Excel</div>
                  <div class="text-body-2 text-medium-emphasis">Download all suppliers as a formatted .xlsx file</div>
                </div>
              </div>
          ...
          <v-col cols="12" lg="5">
            <v-card rounded="xl" flat border class="pa-6 h-100">
              ...
              <div v-if="activeFilterChips.length === 0" class="text-body-2 text-medium-emphasis pa-4 text-center">No filters active — export will include all suppliers.</div>
              ...
              <v-alert type="info" variant="tonal" density="compact" class="mt-5" rounded="lg">
                Filters carried over from the Suppliers page. Return there to change them, then re-open Import / Export.
              </v-alert>
              <v-btn variant="text" color="primary" prepend-icon="mdi-arrow-left" class="mt-2" @click="navigateTo('/suppliers')">Back to Suppliers</v-btn>
            </v-card>
          </v-col>
        </v-row>
      </v-window-item>
...
function addRow() {
  previewRows.value.push({
    ...
    city: '', country: 'United States', payment_terms: '', lead_time_days: 7,
    is_active: true,
  })
}
...
async function saveBulk() {
  ...
  if (invalid > 0) { toast.error(`${invalid} row(s) missing a required Name — fix or remove them`); return }
  saving.value = true
...
async function doExport() {
  ...
  } catch (e) { toast.error('Failed to export suppliers') }
