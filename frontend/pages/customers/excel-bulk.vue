        ...
        <div class="d-flex align-center ga-3">
          ...
          <div>
            ...
            <div class="text-body-2 text-medium-emphasis">
              Bulk upload &amp; export customers via .xlsx workbook
            </div>
          </div>
        </div>
      </v-col>
      <v-col cols="12" sm="5" class="d-flex justify-end ga-2 flex-wrap">
        <v-btn variant="outlined" prepend-icon="mdi-account-group-outline" @click="navigateTo('/customers')">
          View Customers
        </v-btn>
      </v-col>
    </v-row>
      ...
      <v-window-item value="import">
        ...
        <v-card rounded="xl" flat border class="pa-6 mb-6">
          ...
          <v-alert type="info" variant="tonal" density="compact" rounded="lg">
            Each row maps to one customer. <strong>Customer Code is optional</strong> — blank codes are auto-generated. Existing codes are updated.
          </v-alert>
        </v-card>
        ...
        <v-card rounded="xl" flat border class="pa-6 mb-6">
          ...
          >
            ...
            <template v-if="!selectedFile">
              ...
              <div class="text-body-2 text-medium-emphasis">Only .xlsx files · max 20MB · Customer Code optional</div>
            </template>
            <template v-else>
          ...
          <v-card v-if="preview" rounded="xl" flat border class="pa-6 mb-6">
            <div class="d-flex align-center ga-3 mb-5 flex-wrap">
              ...
              <div>
                ...
                <div class="text-body-2 text-medium-emphasis">
                  {{ previewRows.length }} row(s) · {{ preview.skipped }} empty row(s) skipped · click any cell to edit
                </div>
              </div>
              <v-spacer />
            ...
            <v-alert v-if="parseErrors.length > 0" type="warning" variant="tonal" density="compact" class="mb-4" rounded="lg">
              {{ parseErrors.length }} parse warning(s). Rows with issues are highlighted — fix or remove them before saving.
            </v-alert>
              ...
              <v-table density="compact" class="preview-table">
                <thead class="bg-grey-lighten-4">
                  <tr>
                    ...
                    <th style="min-width: 100px;">Loyalty Tier</th>
                ...
                <tbody>
                  <tr v-for="(row, i) in previewRows" :key="i"
                    ...
                    <td><v-text-field v-model="row.customer_code" variant="outlined" density="compact" hide-details placeholder="auto" class="preview-input" /></td>
                    <td>
                      <v-select v-model="row.customer_type" :items="customerTypeOptions" variant="outlined" density="compact" hide-details class="preview-input" />
                    </td>
                    <td><v-text-field v-model="row.first_name" variant="outlined" density="compact" hide-details class="preview-input" /></td>
                    <td><v-text-field v-model="row.last_name" variant="outlined" density="compact" hide-details class="preview-input" /></td>
                    <td><v-text-field v-model="row.company_name" variant="outlined" density="compact" hide-details class="preview-input" /></td>
                    <td><v-text-field v-model="row.email" variant="outlined" density="compact" hide-details class="preview-input" /></td>
                    <td><v-text-field v-model="row.phone" variant="outlined" density="compact" hide-details class="preview-input" /></td>
                    <td><v-text-field v-model="row.city" variant="outlined" density="compact" hide-details class="preview-input" /></td>
                    <td><v-text-field v-model="row.country" variant="outlined" density="compact" hide-details class="preview-input" /></td>
                    <td><v-text-field v-model="row.loyalty_tier" variant="outlined" density="compact" hide-details class="preview-input" /></td>
                    <td><v-text-field v-model="row.credit_limit" type="number" step="0.01" variant="outlined" density="compact" hide-details class="preview-input" /></td>
                    <td><v-switch v-model="row.is_active" color="success" density="compact" hide-details inset /></td>
            ...
            <div class="d-flex align-center ga-3 mt-5 flex-wrap">
              <v-btn color="success" prepend-icon="mdi-content-save" :loading="saving"
                ...
                Save {{ previewRows.length }} Customer(s)
              </v-btn>
              <span v-if="emptyCodeCount > 0" class="text-body-2 text-medium-emphasis">
                {{ emptyCodeCount }} row(s) will get auto-generated codes when saved
              </span>
            </div>
          </v-card>
        </v-scale-transition>
          ...
          <v-card v-if="importResult" rounded="xl" flat border class="pa-6" :style="resultBorderStyle">
            <div class="d-flex align-center ga-3 mb-5 flex-wrap">
              ...
              <div>
                ...
                <div class="text-body-2 text-medium-emphasis">{{ importResult.total_processed }} row(s) processed</div>
              </div>
              <v-spacer />
              <v-btn v-if="importResult.failed === 0" color="success" prepend-icon="mdi-account-group-outline" @click="navigateTo('/customers?imported=1')">
                View Customers
              </v-btn>
              <v-btn variant="text" prepend-icon="mdi-refresh" @click="resetAll">Start Over</v-btn>
            </div>
            <v-row density="comfortable" class="mb-5">
              <v-col cols="6" sm="3"><v-card variant="outlined" rounded="lg" class="pa-4 text-center" flat><div class="text-caption text-medium-emphasis text-uppercase">Created</div><div class="text-h4 font-weight-bold text-success mt-1">{{ importResult.created }}</div></v-card></v-col>
              <v-col cols="6" sm="3"><v-card variant="outlined" rounded="lg" class="pa-4 text-center" flat><div class="text-caption text-medium-emphasis text-uppercase">Updated</div><div class="text-h4 font-weight-bold text-primary mt-1">{{ importResult.updated }}</div></v-card></v-col>
        ...
        <v-row>
          <v-col cols="12" lg="7">
            <v-card rounded="xl" flat border class="pa-6 h-100">
              <div class="d-flex align-center ga-3 mb-5">
                ...
                <div>
                  <div class="text-h6 font-weight-bold">Export Customers to Excel</div>
                  <div class="text-body-2 text-medium-emphasis">Download all customers as a formatted .xlsx file</div>
                </div>
              </div>
          ...
          <v-col cols="12" lg="5">
            <v-card rounded="xl" flat border class="pa-6 h-100">
              ...
              <div v-if="activeFilterChips.length === 0" class="text-body-2 text-medium-emphasis pa-4 text-center">No filters active — export will include all customers.</div>
              ...
              <v-alert type="info" variant="tonal" density="compact" class="mt-5" rounded="lg">
                Filters carried over from the Customers page. Return there to change them, then re-open Import / Export.
              </v-alert>
              <v-btn variant="text" color="primary" prepend-icon="mdi-arrow-left" class="mt-2" @click="navigateTo('/customers')">Back to Customers</v-btn>
            </v-card>
          </v-col>
        </v-row>
      </v-window-item>
...
function addRow() {
  previewRows.value.push({
    customer_code: '', customer_type: 'individual', first_name: '', last_name: '',
    company_name: '', email: '', phone: '', city: '', country: 'United States',
    loyalty_tier: 'bronze', credit_limit: 0, is_active: true,
  })
}
...
async function saveBulk() {
  ...
  if (invalid > 0) { toast.error(`${invalid} row(s) missing a name — fix or remove them`); return }
  saving.value = true
...
async function doExport() {
  ...
  } catch (e) { toast.error('Failed to export customers') }
